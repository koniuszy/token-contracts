...
contract PresalePublic is Ownable {
    ...
    address[] private _allowedParticipants;
    ...
    constructor(
        ...
        _totalAllocated = 0;
        _saleStart = 0;
        _isEveryoneAllowedToParticipateAfterDelay = isEveryoneAllowedToParticipateAfterDelayValue;
        _wasClosed = false;
        _wasStarted = false;
        _allowedParticipants = allowedParticipantsValue;
    }
    ...
    function allocate() public payable {
        ...
        require(
            ...
            "PresalePublic: Cannot allocate anymore"
        );
        require(
            (msg.value >= _minimumAllocation),
            "PresalePublic: Allocation is too small"
        );
        require(
            ((msg.value + _allocations[msg.sender]) <= _maximumAllocation),
            "PresalePublic: Allocation is too big"
        );
        require(
            canAllocate(msg.sender),
            "PresalePublic: Not allowed to participate"
        );
     ...
     */
    function addAllowedParticipants(address[] memory participantsValue)
        public
        onlyOwner
    {
        require(
            areAllocationsAccepted(),
            "PresalePublic: Allocations were already closed"
        );

        for (uint256 i = 0; i < participantsValue.length; ++i) {
            _allowedParticipants.push(participantsValue[i]);
        }
    }
    ...
    function areAllocationsAccepted() public view returns (bool) {
        return (isActive() &&
            (_totalAllocationsLimit - _totalAllocated) >= _minimumAllocation);
    }
    ...
    function canAllocate(address participant) public view returns (bool) {
        ...
        return ((_allocations[participant] + _minimumAllocation) <=
            _maximumAllocation);
    }
    ...
    function canCloseSale() public view returns (bool) {
        return (isActive() &&
            (!areAllocationsAccepted() ||
                _closingAllocationsRemainder >=
                (_totalAllocationsLimit - _totalAllocated) ||
     ...
     */
    function isAllowedToParticipate(address participant)
        public
        view
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < _allowedParticipants.length; ++i) {
            if (_allowedParticipants[i] == participant) {
                return true;
            }
        }
     ...
     */
    function isEveryoneAllowedToParticipateAfterDelay()
        public
        view
        returns (bool)
    {
